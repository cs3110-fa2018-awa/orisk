...
type t = {id : int; name : string; color : color;
          artificial : bool; personality : Personality.t}
...
let create name color artificial =
  {id = id_counter (); name = name; color = color;
   artificial = artificial; personality = Personality.random ()}
...
let json_of_player player =
  `Assoc [
    ...
    ("personality", json_of_personality player.personality);
  ]
